...
if __name__ == '__main__':
    ...
    myValuation = 0.8
    othersValuation = 0.6
    bids = np.arange(0, 1, 0.01) #ATTENZIONE ALLA DISCRETIZZAZIONE
    # numBids = len(bids)
    numBids = 1000
    nUsers = 1000
    B = 100
    ...
    win_probabilities = stats.beta.cdf(bids, nAdvertisers, 1)
    ...
    expected_clairvoyant_bids = [sum(bids * gamma * win_probabilities) for u in range(nUsers)]
    ...
    for u in range(nUsers):
        ...
        myWin = int(winners == 0)
        f_t, c_t = (myValuation - m_t[u]) * myWin, m_t[u] * myWin
